...
@Suite struct RTMPChunkBufferTests {
    ...
    @Test func writeChunkSize() {
        ...
        let data = buffer.putMessage(.zero, chunkStreamId: 1, message: connection)
        #expect(data.count == 210)
        #expect(Data(data.prefix(12)) == Data([1, 0, 0, 0, 0, 0, 197, 20, 0, 0, 0, 0]))
        #expect(Data(data[12..<15]) == Data([2, 0, 184]))
        #expect(data[140] == 193)
        #expect(data[12..<140].count == 128)
        #expect(data[141..<210].count == 69)
    }
